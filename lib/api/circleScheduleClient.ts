...
- 각 약속을 한국어 "한 문장"으로 간결히 요약한다. 약속 내용이 핵심이며 명사형 종결을 권장한다. (예: "할머니 생신 가족 모임")
- 시간/날짜 표현이 있으면 그 문장의 맨 뒤에 괄호로 덧붙인다. 예: "할머니 생신 가족 모임 (6/28 오후 2시)". 시간이 없으면 아무것도 붙이지 않는다.
- 상대 표현("담주 토욜", "모레")은 주어진 오늘 날짜 기준으로 확실할 때만 환산하고, 애매하면 사용자가 쓴 표현을 그대로 둔다.
- 정렬용 when_at(ISO8601, +09:00)을 추정할 수 있으면 채우고, 없으면 null로 둔다.
- 대상/카테고리는 절대 판단하지 않는다(사용자가 직접 지정한다).

출력: 아래 JSON만 출력한다. 설명·마크다운·코드펜스 금지.
{"appointments":[{"content":"<한 문장>","when_at":"<ISO 또는 null>","source":"<원본 근거 텍스트>"}]}
